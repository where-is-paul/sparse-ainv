...
template <class el_type> inline void lilc_matrix<el_type>::find_root(int &s) {
  ...
  while (true) {
    ...
    if (ls > ls_max) {
      ...
      for (idx_it it = lvl_set.begin(); it != lvl_set.end(); it++) {
        ...
        if (m_idx[*it].size() > 0 && m_idx[*it][0] == *it)
          deg--;
        if (deg <
            min_deg) { // should consider tie breaking by index later if needed.
          min_deg = deg;
          s = *it;
        }
      }
