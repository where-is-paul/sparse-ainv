...
inline int tdfs(int j, int k, int *head, const int *next, int *post,
                int *stack) {
  int i, p, top = 0;
...
* SparseMatrix object. Both the upper and lower parts have to be stored, but the
* diagonal entries are optional.
...
inline void lilc_matrix<el_type>::sym_amd(vector<int> &perm) {
  ...
  /* --- Postordering ----------------------------------------------------- */
  for (i = 0; i < n; i++)
    Cp[i] = amd::amd_flip(Cp[i]); /* fix assembly tree */
  for (j = 0; j <= n; j++)
    head[j] = -1;
  ...
  {
    if (Cp[i] == -1)
      k = amd::tdfs(i, k, head, next, temp_perm, w);
  }
