...
// DEFINE_double(fill, 3.0, "A parameter to control memory usage. Each column is
// guaranteed"
//		"to have fewer than fill*nnz(A) elements.");

DEFINE_double(tol, 0.001,
              "A parameter to control agressiveness of dropping. If "
              "'relative', then in each column k,"
              "elements less than tol*||L(k+1:n,k)|| (1-norm) are dropped. If "
              "'absolute', each element less than tol is dropped.");

DEFINE_double(
    beta, 1.0,
    "A parameter to aggressiveness of Bunch-Kaufman pivoting (BKP). "
    "When pp_tol >= 1, full BKP is used. When pp_tol is 0, BKP is faster"
    "but chooses poorer pivots. Values between 0 and 1 varies the "
    "aggressiveness of"
    "BKP in a continuous manner.");

DEFINE_string(pivot, "bunch",
              "Determines what kind of pivoting algorithm will be used"
...
DEFINE_string(
    equil, "bunch",
    "Decides if the matrix should be equilibriated before factoring is done. "
    "Options are 'bunch' and 'none'. If the option is 'bunch', the matrix is "
    "equilibrated "
    "with Bunch's algorithm in the max norm. The default is 'bunch'.");

DEFINE_bool(save, true,
            "If yes, saves the factors (in matrix-market format) into a folder "
            "called output_matrices/ in the same directory as ldl_driver.");

#ifdef SYM_ILDL_DEBUG
DEFINE_bool(display, false,
            "If yes, outputs a human readable version of the factors onto"
...
DEFINE_int32(
    max_iters, -1,
    "If >= 0 and supplied with a right hand side, SYM-ILDL will attempt "
...
DEFINE_string(solver, "sqmr",
              "The solver used if supplied a right-hand side. The "
              "solution will be written to output_matrices/ in matrix-market "
              "format. Choices are 'sqmr', 'minres', and 'full'");

DEFINE_double(solver_tol, 1e-6, "A tolerance for the iterative solver used. "
                                "When the iterate x satisfies ||Ax-b||/||b|| < "
                                "solver_tol, the solver is terminated. Has no "
                                "effect when doing a full solve.");

DEFINE_string(rhs_file, "",
              "The filename of the right hand side (in matrix-market format).");

DEFINE_string(drop_type, "absolute", "The type of drop tolerance used for "
                                     "removing small elements. Options are "
                                     "'relative' or 'absolute'.");

int main(int argc, char *argv[]) {
  std::string usage(
      "Performs an incomplete LDL factorization of a given matrix.\n"
      "Sample usage:\n"
  ...
  if (FLAGS_filename.empty()) {
    std::cerr << "No file specified! Type ./ldl_driver --help for a "
  ...
  if (FLAGS_max_iters > 0 || !FLAGS_rhs_file.empty()) {
    if (FLAGS_max_iters <= 0) {
      printf("Using SQMR (200 max iterations) as default solver since RHS was "
             "loaded.\n");
      FLAGS_max_iters = 200;
    }
    ...
    if (rhs.size() != solv.A.n_cols()) {
      std::cout
          << "The right hand side dimensions do not match the dimensions of A."
          << std::endl;
      return 1;
    }
    solv.set_rhs(rhs);
