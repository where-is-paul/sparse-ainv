...
template <class el_type> class wmn_pivoter : public pivot_strategy<el_type> {
public:
  wmn_pivoter(lilc_matrix<el_type> *A_, lilc_matrix<el_type> *L_,
  ...
  pivot_struct find_pivot(int col) {
    ...
    if (std::max(std::abs(a11), w1) <= this->m_bound) {
      ...
      double nl1 = norm(this->L->m_x[col], 2.0),
             nlr = norm(this->L->m_x[r], 2.0);
      ga = norm(this->A1, this->A1_idx, 2.0) /
           std::max(std::abs(a11), this->m_eps);
      gb = norm(this->Ar, this->Ar_idx, 2.0) /
           std::max(std::abs(arr), this->m_eps);
      ...
      if (gb < eta * std::min(gamma * ga, gc) ||
          nlr * gb < eta * std::min(gamma * nl1 * ga, nl1r * gc)) {
        this->A1.swap(this->Ar);
        this->A1_idx.swap(this->Ar_idx);
        return pivot_struct(false, r);
      } else if (std::abs(a11) == 0 || gc < gamma * ga ||
                 nl1r * gc < gamma * nl1 * ga) {
