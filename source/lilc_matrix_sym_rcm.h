...
namespace {
/*! \brief Functor for comparing elements by degree (in increasing order)
   instead of by index.
        \param A a pointer to the matrix being reordered.
*/
template <class el_type> struct by_degree {
  ...
  bool operator()(int const &a, int const &b) const {
    ...
    if (A->m_idx[a].size() > 0 && A->m_idx[a][0] == a)
      deg_a--;
    if (A->m_idx[b].size() > 0 && A->m_idx[b][0] == b)
      deg_b--;
