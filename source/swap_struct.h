...
/*!	\brief A structure containing variables used in pivoting a LIL-C matrix.
        ...
        Storing these variables in a combined structure reduces memory
   requirements and bundles together all temporary structures needed during
   pivoting.
*/
template <class el_type> class swap_struct {
...
public:
  vector<idx_it>
      swapk; ///<List of indices from row r that will be swapped to row k.
  vector<idx_it>
      swapr; ///<List of indices from row k that will be swapped to row r.

  idx_vector_type
      all_swaps; ///<Column indices of all swaps done in swapk and swapr.
